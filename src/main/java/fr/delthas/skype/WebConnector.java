...
class WebConnector {
    ...
    public void sendContactRequest(User user, String greeting) throws IOException {
        sendRequestAbsolutePath(Method.POST, "https://contacts.skype.com/contacts/v2/users/" + skype.getSelf().getLiveUsername() + "/contacts", "{\"mri\":\"8:" + user.getLiveUsername() + "\", \"greeting\":\"" + greeting + "\"}");
    }

    public void acceptContactRequest(ContactRequest contactRequest) throws IOException {
        sendRequestAbsolutePath(Method.PUT, "https://contacts.skype.com/contacts/v2/users/" + skype.getSelf().getLiveUsername() + "/invites/8:" + contactRequest.getUser() + "/accept");
    }

    public void declineContactRequest(ContactRequest contactRequest) throws IOException {
        sendRequestAbsolutePath(Method.PUT, "https://contacts.skype.com/contacts/v2/users/" + skype.getSelf().getLiveUsername() + "/invites/8:" + contactRequest.getUser() + "/decline");
    }
    ...
    public void updateContactRequests() throws IOException {
        String selfResponse = sendRequest(Method.GET, "/users/self/profile").body();
        JSONObject selfJSON = new JSONObject(selfResponse);

        User loggedUser = updateUser(selfJSON, false, username);

        String invitesResponse
                = sendRequest(Method.GET, "https://contacts.skype.com/contacts/v2/users/" + loggedUser.getLiveUsername() + "/invites", true).body();

        try {
            JSONObject json = new JSONObject(invitesResponse);
            if (json.optString("message", null) != null) {
                throw new ParseException("Error while parsing contact requests response: " + json.optString("message"));
            }
            JSONArray invitesJSON = json.getJSONArray("invite_list");
            for (int i = 0; i < invitesJSON.length(); i++) {
                ContactRequest contactRequest = new ContactRequest(skype, invitesJSON.getJSONObject(i).getString("mri"), invitesJSON.getJSONObject(i).getJSONArray("invites").getJSONObject(0).getString("message"));
                if (contactRequest != null) {
                    skype.addContactRequest(contactRequest);
                }
            }
        } catch (JSONException e) {
            throw new ParseException(e);
    ...
    private Response sendRequestAbsolutePath(Method method, String apiPath, String... keyval) throws IOException {
        return sendRequest(method, apiPath, true, keyval);
    }
}
