 ...
 * <p>
 * All IO exceptions that might be thrown by any action are catched and passed
 * to the registered error listener and disconnect the interface immediately.
 * <p>
 * <b>Note:</b> All strings passed to user/group/skype objects will have their
 * control characters removed (0x00-0x1F and 0x7F-0x9F) except for CR and LF
 * which will be replaced with CRLF if needed.
 * <p>
 * <b>If you want to report a bug, please enable debug/logs with
 * {@link Skype#setDebug(Path)}, before using {@link #connect()}.</b>
 */
public final class Skype {
    ...
    private boolean autoAcceptContactRequests;
     ...
     */
    public Skype(String username, String password, boolean autoAcceptContactRequests) {
        this.username = username;
        this.password = password;
        this.autoAcceptContactRequests = autoAcceptContactRequests;
    ...
    /**
     * Enables or disables debug of the Skype library (globally). (By default
     * logs are <b>disabled</b>.)
     * <p>
     * If enabled, debug information and logs will be written to a log file at
     * the specified path. If the path is null, the debug will be disabled.
     *
     * @param path The path at which to write debugging information, or null to
     * disable logging.
     * @throws IOException may be thrown when adding a file handler to the
     * logger
     */
     ...
     *
     * @param presence The initial presence of the Skype account after
     * connection. Cannot be {@link Presence#OFFLINE}.
     * @throws IOException If an error is thrown while connecting.
     * @throws InterruptedException If the connection is interrupted.
     */
    public void connect(Presence presence) throws IOException, InterruptedException {
        ...
        if (autoAcceptContactRequests) {
            getContactRequests();
            acceptAllContactRequests();
        } else {
            getContactRequests();
        }
    }
     ...
     * <p>
     * All User, Group, and ContactRequest objects will remain valid for the
     * next connections. Make sure to reconnect (start) before triggering
     * actions from these, however, or they will throw an IllegalStateException.
     */
    ...
    /**
     * @return The current list of contact requests to this Skype account
     * (snapshot, won't be updated).
     */
    public List<ContactRequest> getContactRequests() {
        ensureConnected();
        try {
            webConnector.updateContactRequests();
        } catch (IOException e) {
            error(e);
        }
    ...
    /**
     * @return The groups (or threads) the Skype account is currently in (as a
     * snapshot: the list won't be updated).
     */
    ...
    /**
     * @return The current list of contacts of the account (snapshot, won't be
     * updated).
     */
     ...
     * <p>
     * All presence values are valid except {@link Presence#OFFLINE} : to
     * disconnect, use {@link #disconnect()}.
     *
    ...
    public boolean isAutoAcceptContactRequestsEnabled() {
        return autoAcceptContactRequests;
    }

    public void acceptAllContactRequests() {
    ...
    void addContactRequest(ContactRequest contactRequest) {
        if (!contactRequests.contains(contactRequest)) {
            logger.finest("Adding contact request " + contactRequest.toString());
